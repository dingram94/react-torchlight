...
export default function createPlugin(
  ...
  function extractTourIds(sourceFile: ts.SourceFile) {
    function visit(node: ts.Node) {
      // Look for useSpotlightSteps calls
      if (ts.isCallExpression(node)) {
        const expression = node.expression;

        // Handle direct calls: useSpotlightSteps("tourId", ...)
        if (
          ts.isIdentifier(expression) &&
          expression.text === "useSpotlightSteps"
        ) {
        ...
        // Handle property access: spotlight.useSpotlightSteps("tourId", ...)
        if (
          ts.isPropertyAccessExpression(expression) &&
          ts.isIdentifier(expression.name) &&
          expression.name.text === "useSpotlightSteps"
        ) {
          const firstArg = node.arguments[0];
        ...
        // Handle imported useSpotlightSteps calls
        if (ts.isCallExpression(node) && node.arguments.length >= 1) {
          const firstArg = node.arguments[0];
          if (ts.isStringLiteral(firstArg) && firstArg.text.trim() !== "") {
            // Check if this is likely a useSpotlightSteps call by looking at the expression
            const expressionText = node.expression.getText();
            if (expressionText.includes("useSpotlightSteps")) {
              tourIds.add(firstArg.text);
            }
          }
        }
  ...
  function generateTypeFile() {
    ...
    const content = `// Auto-generated by tour TypeScript plugin
...
export function createTransformer(
  ...
  const outputPath = config.outputPath || "generated-tour-types.d.ts";
    ...
    return (sourceFile: ts.SourceFile) => {
      function visit(node: ts.Node): ts.Node {
        if (ts.isCallExpression(node)) {
          ...
          // Handle useSpotlightSteps calls
          if (
            ts.isIdentifier(expression) &&
            expression.text === "useSpotlightSteps"
          ) {
