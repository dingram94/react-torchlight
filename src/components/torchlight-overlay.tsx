...
import { useTorchlight } from "../context";
import { TorchlightOverlayProps } from "../types";

export const TorchlightOverlay: React.FC<TorchlightOverlayProps> = ({
  className = "",
  overlayColor = "#000000",
  overlayOpacity = 0.7,
  highlightPadding = 8,
  ...
  return (
    <div className={`torchlight-overlay ${className}`}>
      {/* Dark overlay with cutout */}
      <svg className="torchlight-overlay__svg">
        <defs>
          <mask id="torchlight-mask">
      ...
      {showTooltip && (currentStep.title || currentStep.content) && (
        <div
          className={`torchlight-tooltip ${tooltipClassName}`}
          style={tooltipStyle}
        >
          {currentStep.title && (
            <h3 className="torchlight-tooltip__title">{currentStep.title}</h3>
          )}
          {currentStep.content && (
            <p className="torchlight-tooltip__content">{currentStep.content}</p>
          )}

          <div className="torchlight-tooltip__footer">
            <div className="torchlight-tooltip__step-counter">
              {currentTour.currentStepIndex + 1} of {currentTour.steps.length}
            </div>

            <div className="torchlight-tooltip__button-group">
              ...
              <button
                className="torchlight-tooltip__button--primary"
      ...
      return {
        left: Math.max(
          ...
          Math.min(
            ...
            highlightRect.left + highlightRect.width / 2 / 2
          )
        ),
